...
router.put('/event/:facultyId/:eventId', upload.array('pic', 1) , async (req: any, res: any, next: NextFunction) => {
    ...
    try {
        ...
        const  updateimage = req.files['pic'] ? req.files['pic'][0].filename : eventToUpdate.pic
        ...
        // Update the event image if a new image is uploaded
...
router.get('/faculty/:facultyId/:eventId', async (req: Request, res: Response, next: NextFunction) => {
    const { facultyId, eventId } = req.params;
    try {
        ...
        if (event.amount) {
            const events = await Registerevent.find({ event: eventId });
            console.log(events);

            price = events.map((item: any) => item.amount.price);
            console.log(price);
            ...
            res.status(404).json({
                success:false,
                message:"in this event no payment "
            })
        }
