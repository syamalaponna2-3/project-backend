...
const Quire = require('../models/Quires')
...
router.post('/registerUser', uploads.fields([{ name: 'Avatar', maxCount: 1 }]), async (req: any, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        res.status(201).json({ success: true, message: 'User registered successfully. Check your email for verification.',newUser });
    } catch (error) {
        console.error(error);
...
router.put('/student/:studentId', uploads.fields([{ name: 'Avathar', maxCount: 1 }]), async (req: any, res: any, next: NextFunction) => {
    const studentId = req.params.studentId;
    try {
        ...
        const updatedImage = req.files['Avathar'] ? req.files['Avathar'][0].filename : existingUser.image;

        // Update the user's information

        const updatedUser = await user.findByIdAndUpdate(studentId,{
            email, mobileNumber, name,
            Avathar:updatedImage,
        },{new:true})
       if(updatedUser){
        res.status(200).json({
            success: true,
...
router.get('/registerevent/:studentId',async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        console.log(eventid)
        const registerd = await Event.find({_id:eventid})
...
router.get('/history/:studentId',async(req:Request,res:Response,next:NextFunction)=>{
    ...
    try{
        ...
        const events =  await Registerevent.find({student:studentId,to_date:{$lt:currectdate} });
        if(events.length > 0){
            res.status(200).json({
                success:true,
                events
...
router.post('/event/:studentId/:eventId', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const events = await Registerevent.find({ student: studentId,event:eventId });
        if (!events) {
            res.status(404).json({
                success: false,
                message: "no event register"
            })
        }
        const feedback  = req.body;
        const currentDate = Date.now();
        const time = await events.filter((event:any)=> event.end_time.getTime() > currentDate)
        if(time){
...
router.post('/quires/:studentId',async(req:Request,res:Response,next:NextFunction)=>{
    ...
    try{
        ...
        if(!student){
            res.status(404).json({
                success:false,
                message:"no student not found in this id"
            })
        }
...
//     try {
//         const { year, department, price, others, clubs } = req.query as { year?: string; department?: string; price?: string; others?: string; clubs?: string };
//         let query = {};
...
//         }
//         if (price) {
//             const [priceMin, priceMax] = price.split('-').map(p => parseInt(p.trim()));
