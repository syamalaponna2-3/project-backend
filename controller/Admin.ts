...
router.get('/student', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const student = req.query.student as string | undefined;
        if (!student) {
            return res.status(400).json({
                success: false,
                message: "Please provide a valid student query parameter"
            });
        }
        
        const users = await user.find({ name: { $regex: new RegExp(student, 'i') } });
        if (users.length === 0) {
            return res.status(404).json({
                success: false,
                message: "No students found"
...
router.get('/allevents', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const title = req.query.title as string;
        const events = await Event.find({ title: { $regex: new RegExp(title, 'i') } });
...
router.get('/filter', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { year, department, amount, clubs, others } = req.query;
        ...
        if (amount) {
            // Split the amount query parameter into min and max amounts
            const [minAmount, maxAmount] = (amount as string).split('-').map(Number);
        ...
        const filteredEvents = await events.filter((item:any)=>{
...
router.get('/quires', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const quire = await Problems.find();
        if (quire.length === 0) {
            return res.status(404).json({
                success: false,
